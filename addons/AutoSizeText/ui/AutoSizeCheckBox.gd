...
func _ready() -> void:
	...
	if _label == null:
		...
		_label.size = Vector2(size.x - margin, size.y)
...
func _get_biggest_check_icon_size() -> Vector2:
	...
	for icon_name in icons_to_check:
		var theme_icon: Texture2D = get_theme_icon(icon_name, "CheckBox")
		if theme_icon == null:
			continue
...
func _get_icon_space() -> int:
	var margin: int = 0
	margin = get_theme_constant("h_separation", "CheckBox")
	
	var stylebox = get_theme_stylebox("normal", "CheckBox")
